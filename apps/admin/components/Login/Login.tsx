...
import { fetchLoginAdmin } from "services";
...
export const Login: FunctionComponent<ILogin> = ({ token }) => {
	...
	const handleLogin = async () => {
		try {
			const data = await fetchLoginAdmin(email, password);
			await getUserInfos(data?.admin, token);
			// toast({
			// 	id: "toast-login-suc",
			// 	position: "top-right",
			// 	status: "success",
	...
	return (
		...
		>
			...
			<Flex h="100%" justifyContent="flex-end" flex="1">
				<Img src="backgrounds/LoginBackground.png" h="100%" w="max-content" />
			</Flex>
		</Flex>
	);
};
